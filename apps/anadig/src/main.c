...
#define GROVE_A1_P2      27

struct adc_dev *adc;
static int adc_pro_sec = 4;
...
adc_read_event(struct adc_dev *dev, void *arg, uint8_t etype,
    ...
    int rc = -1;
    int value = adc_nrf51_driver_read(buffer, buffer_len);
    int digital = hal_gpio_read(GROVE_A1_P2) * 1000;
...
main(void) {
    ...
    init_adc_timer();
    start_adc_timer();
    hal_gpio_init_in(GROVE_A1_P2, HAL_GPIO_PULL_DOWN);
