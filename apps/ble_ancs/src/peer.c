                ...
                const struct ble_gatt_chr *chr, void *arg)
{
    ...
    switch (error->status) {
    case 0:
            BLEPRPH_LOG(INFO, "peer_chr_disced status: %d\n", error->status);
            print_bytes(chr->uuid.u128.value, 16);
...
peer_svc_add(struct peer *peer, const struct ble_gatt_svc *gatt_svc)
{
    ...
    BLEPRPH_LOG(INFO, "########### found svc\n");
    svc = os_memblock_get(&peer_svc_pool);
    if (svc == NULL) {
        /* Out of memory. */
        BLEPRPH_LOG(INFO, "#################################### abbruch no memory found svc\n");
                ...
                const struct ble_gatt_svc *service, void *arg)
{
    ...
    switch (error->status) {
    case 0:
        print_bytes(service->uuid.u128.value, 16);
